...
function worldPos(ix, iy, iz, bounds, res) {
  ...
  return new THREE.Vector3(
    bmin[0] + (ix / (res - 1)) * (bmax[0] - bmin[0]),
    bmin[1] + (iy / (res - 1)) * (bmax[1] - bmin[1]),
    bmin[2] + (iz / (res - 1)) * (bmax[2] - bmin[2])
  )
}

function interpolateVertex(p1, p2, v1, v2, level) {
  if (Math.abs(v2 - v1) < 1e-30) return p1.clone()
  const t = (level - v1) / (v2 - v1)
  return new THREE.Vector3().lerpVectors(p1, p2, Math.max(0, Math.min(1, t)))
}

export function extractIsosurface(gridData, level, bounds) {
  const { grid, nx, ny, nz } = gridData
  const positions = []
  const normals = []

  const getVal = (ix, iy, iz) => {
    if (ix < 0 || ix >= nx || iy < 0 || iy >= ny || iz < 0 || iz >= nz) return level
    return grid[iz * nx * ny + iy * nx + ix]
  }
    ...
    for (let iy = 0; iy < ny - 1; iy++) {
      for (let ix = 0; ix < nx - 1; ix++) {
        ...
        for (let i = 0; i < 8; i++) {
          ...
          cornerVals.push(getVal(ix + dx, iy + dy, iz + dz))
          cornerWPos.push(worldPos(ix + dx, iy + dy, iz + dz, bounds, nx))
        }

        let cubeIndex = 0
        for (let i = 0; i < 8; i++) {
          if (cornerVals[i] >= level) cubeIndex |= (1 << i)
        }
        ...
        for (let e = 0; e < 12; e++) {
          if (edgeTable[cubeIndex] & (1 << e)) {
            const [ea, eb] = edgeVertices[e]
            vertList[e] = interpolateVertex(cornerWPos[ea], cornerWPos[eb], cornerVals[ea], cornerVals[eb], level)
          }
        }
...
export function buildIsosurfaceGeometry(gridData, level, bounds) {
  const mesh = extractIsosurface(gridData, level, bounds)
